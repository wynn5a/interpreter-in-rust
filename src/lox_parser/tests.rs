                    ...
                    match while_stmt.body.as_ref() {
                        StmtEnum::Block(body_block) => {
                            ...
                            match &body_block.statements[1] {
                                StmtEnum::Expression(_) => {}
                                _ => {
                                    panic!("Expected Expression statement (increment) in loop body")
                                }
                            }
                        }
...
fn test_parse_return_with_value() {
    ...
    match &statements[0] {
        StmtEnum::Return(ret_stmt) => {
            ...
            match ret_stmt.value.as_ref().unwrap().as_ref() {
                ExprEnum::Literal(l) => match &l.value {
...
fn test_parse_multiple_returns_in_if() {
    ...
    let tokens = vec![
        Token::new(TokenType::Fun, "fun".to_string(), None, 1),
        Token::new(TokenType::Identifier, "max".to_string(), None, 1),
        Token::new(TokenType::LeftParen, "(".to_string(), None, 1),
        Token::new(TokenType::Identifier, "a".to_string(), None, 1),
        Token::new(TokenType::Comma, ",".to_string(), None, 1),
        Token::new(TokenType::Identifier, "b".to_string(), None, 1),
    ...
    match &statements[0] {
        StmtEnum::Function(func_stmt) => {
            assert_eq!(func_stmt.body.len(), 2);
            match &func_stmt.body[0] {
                StmtEnum::If(_) => {},
                _ => panic!("Expected if statement"),
            }
            match &func_stmt.body[1] {
                StmtEnum::Return(_) => {},
                _ => panic!("Expected return statement"),
            }
        }
