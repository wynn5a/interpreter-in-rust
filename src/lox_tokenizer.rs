...
impl TokenizerState {
    ...
    fn handle_identifier(&mut self) {
        ...
        while !self.is_at_end()
            && (self.peek().unwrap().is_alphanumeric() || self.peek() == Some('_'))
        {
            self.advance();
        }
...
fn tokenize(lox: &mut LoxTokenizer, input: &str) -> Vec<Token> {
    ...
    while !state.is_at_end() {
        ...
        match c {
            ...
            '>' => state.handle_two_char_operator(
                TokenType::Greater,
            ...
            _ => {
                eprintln!("[line {}] Error: Unexpected character: {}", state.line, c);
                lox.had_error = true;
            }
        }
    }
