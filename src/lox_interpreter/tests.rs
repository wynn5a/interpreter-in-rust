...
// =========================================================================
// PHASE 10: FUNCTION TESTS
// =========================================================================

#[test]
fn test_interpret_function_declaration() {
    use crate::lox_parser::LoxParser;
    use crate::lox_tokenizer::LoxTokenizer;
    ...
    // Verify 'foo' is in the environment
    let foo = interpreter.environment.borrow().borrow().get("foo");
    assert!(foo.is_ok());
    match foo.unwrap() {
        LoxValue::Callable(_) => {}
...
#[test]
fn test_interpret_function_call_side_effect() {
    use crate::lox_parser::LoxParser;
    use crate::lox_tokenizer::LoxTokenizer;

    // var sideEffect = false;
    // fun trigger() { sideEffect = true; }
    // trigger();
    let source = "var sideEffect = false; fun trigger() { sideEffect = true; } trigger();";
    let mut tokenizer = LoxTokenizer::default();
    let tokens = tokenizer.tokenize(source);
    let mut parser = LoxParser::new(tokens);
    let statements = parser.parse();
    ...
    let val = interpreter.environment.borrow().borrow().get("sideEffect");
    assert_eq!(val.unwrap(), LoxValue::Boolean(true));
}
...
fn test_interpret_function_scope_closure() {
    ...
    // fun check() {
    //   var local = "local";
    //   global = global + " touched";
    // }
    // check();
    let source = r#"var global = "global"; fun check() { global = global + " touched"; } check();"#;
    let mut tokenizer = LoxTokenizer::default();
    let tokens = tokenizer.tokenize(source);
    let mut parser = LoxParser::new(tokens);
    let statements = parser.parse();
    ...
    let fun_decl = StmtEnum::Function(crate::stmt::FunctionStmt {
        ...
        body: vec![
            StmtEnum::Return(crate::stmt::ReturnStmt {
                keyword: Token::new(TokenType::Return, "return".to_string(), None, 1),
                value: Some(Box::new(ExprEnum::Literal(Literal {
                    value: LiteralValue::Number(42.0),
                }))),
            }),
            StmtEnum::Return(crate::stmt::ReturnStmt {
                keyword: Token::new(TokenType::Return, "return".to_string(), None, 2),
                value: Some(Box::new(ExprEnum::Literal(Literal {
                    value: LiteralValue::Number(99.0),
                }))),
            }),
        ],
    });
